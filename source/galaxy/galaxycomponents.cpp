...
void GalaxyComponentBulge::calculateIntensity(RasterPixel* rp, QVector3D& p, GalaxyInstance* gi, float weight) {
           ...
           // float m_currentRadius = getRadius(p, P,z, gi);
...
void GalaxyComponentDisk::componentIntensity(RasterPixel* rp, QVector3D& p, float ival) {
            ...
            if (ival<0.0005)
				return;
            p2 =abs(getPerlinCloudNoise(p, 1.0f, rp->winding, 9, m_componentParams.scale(), m_componentParams.ks()));
...
void GalaxyComponentDust::componentIntensity(RasterPixel* rp, QVector3D& p, float ival) {
            ...
            float p2 = getPerlinCloudNoise(p, rp->radius, rp->winding, 10, m_componentParams.scale(), m_componentParams.ks());
			//if (p2!=0)
            p2=0.1f/(abs(p2) + 0.1f);
